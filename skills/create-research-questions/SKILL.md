...
- Select the task first: `rpi_get_task_context` with the task slug, or `/rpi-task <slug>`.
...
3. If the area is unfamiliar, optionally run a light `rpi_start_research` fanout (artifact-locator only) to ground the questions in what exists.
